...
class OverflowBinORM(Base):  # Отстойник
    ...
    #сделать id, id продуктов, количество и дата дабавления
    id: Mapped[int] = mapped_column(primary_key=True)
...
class MovementHistoryORM(Base):  # История перемещений (создать)
...
class ProductPlacementORM(Base):  # Размещение товара (запасы)
    __tablename__ = 'product_placements'
     #сделать
    id: Mapped[int] = mapped_column(primary_key=True)
    product_id: Mapped[int] = mapped_column(ForeignKey('products.id'))
    shelf_id: Mapped[Optional[int]] = mapped_column(ForeignKey('shelves.id'), nullable=True)#полка
    overflow_id: Mapped[Optional[int]] = mapped_column(ForeignKey('overflow_bins.id'), nullable=True)#id отстойника
    quantity: Mapped[int] = mapped_column(default=0)
    placement_date: Mapped[datetime] = mapped_column(default=lambda:datetime.now(timezone.utc))#дата когда товар был добавлен 
    last_updated: Mapped[datetime] = mapped_column(default=lambda:datetime.now(timezone.utc), onupdate=lambda:datetime.now(timezone.utc))#последнее уведомление
